...
const AdminBroadcastEdit = () => {
    ...
    const {data: broadcastData, isLoading: isFetchingBroadcast} = useQuery({
        queryKey: ['adminBroadcast', id],
        queryFn: () => broadcastsService.getById(id),
    });

    useEffect(() => {
        if (broadcastData?.data?.broadcast) {
            const b = broadcastData.data.broadcast;
            if (b.status !== 'draft') {
            ...
            setFormData({
                subject: b.subject || '',
                message: b.message || '',
                audience_type: b.audience_type || 'all_users',
                target_university_id: b.target_university_id || '',
                target_event_id: b.target_event_id || '',
                custom_user_ids: b.custom_user_ids || [],
                from_email: b.from_email || '',
                from_name: b.from_name || '',
                reply_to_email: b.reply_to_email || '',
                scheduled_at: b.scheduled_at ? b.scheduled_at.substring(0, 16) : '',
                template: b.template || '',
                track_opens: !!b.track_opens,
                track_clicks: !!b.track_clicks,
            });
        }
    }, [broadcastData, navigate]);
    ...
    const handleSubmit = async (e) => {
        ...
        try {
            await broadcastsService.update(id, formData);
            ...
            if (errors) {
                ...
                toast.error(err.response?.data?.error || 'Failed to update broadcast');
            }
        } finally {
            setIsLoading(false);
        }
    ...
    return (
        <div className="space-y-6">
            <div className="flex items-center gap-4">
                ...
                <div>
                    <h1 className="text-2xl font-bold text-gray-900">Edit Broadcast</h1>
                    <p className="text-gray-600">Update your email campaign details.</p>
                </div>
            </div>
...
export default AdminBroadcastEdit;
